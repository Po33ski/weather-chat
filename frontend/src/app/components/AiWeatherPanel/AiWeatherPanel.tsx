...
export function AiWeatherPanel({ meta, data }: { meta: AiMeta | null; data: AiChatData | null }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Raw data dump (minimal, no extra components) */}
...
// import { normalizeAiPayload } from "@/app/utils/formatAiWeather";
...
//   const kind = normalized?.kind ?? null;
...
//       <div className="space-y-4">
//         <div className="text-sm text-gray-600">
//           {normalized?.meta.city && <span className="font-semibold">{normalized.meta.city}</span>}
//           {normalized?.meta.date && <div>{normalized.meta.date}</div>}
//         </div>
...
//   if ((kind === "forecast" || kind === "history") && Array.isArray((normalized as any)?.days)) {
//     const days: HistoryAndForecastDay[] = ((normalized as any).days || []).map((d: any) => ({
...
//         {normalized?.meta.city && <span className="font-semibold">{normalized.meta.city}</span>}
//         {normalized?.meta.date && <div>{normalized.meta.date}</div>}
//         {normalized?.meta.date_range && <div>{normalized.meta.date_range}</div>}
//         </div>
